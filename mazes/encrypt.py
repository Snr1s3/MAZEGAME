...
# Files to ignore
ignore_files = ['filekeyMaze.key','decrypt.py', 'encrypt.py', 'decrypt.py']

def key():
    # Generate Key
    key = Fernet.generate_key()
    # Save the key in a file
    with open('mazes/filekeyMaze.key', 'wb') as filekeyMaze:
        filekeyMaze.write(key)
def encript_files():
    # opening the key
    with open('mazes/filekeyMaze.key', 'rb') as filekeyMaze:
        key = filekeyMaze.read()
